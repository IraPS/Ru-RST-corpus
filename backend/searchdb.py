...
MARKERS = {"a": "а", "bezuslovno": "безусловно", "buduchi": "будучи",
           ...
           "prietom": "при этом", "priznavat_lem": "признавать", "radi": "ради",
            ...
            for el in i:
                ...
                if ro == ['any']:
                        ...
                        if '_lem' in el['searched_for']:
                            ...
                            if len(marker_rus.split()) > 1:
                                request_on_db += ' REDUCE(s = " ", w in split(n.text_norm, " ")' \
                                                 '[0..{0}]|s + " " + w) CONTAINS \'{1}\' {2}{3}'.\
                                    format(marker_lengh, marker_rus, el['close_parenth'], cond)

                            else:
                                request_on_db += ' \'{0}\' IN split(n.text_norm, " ")[0..{1}] {2}{3}'.\
                                    format(marker_rus, marker_lengh, el['close_parenth'],cond)
                    if el['type'] == 'word':
                        type_chosen = True
                    ...
                    if el['type'] == 'marker':
                        ...
                        if '_lem' in el['searched_for']:
                            request_on_db += ' n.lemmas CONTAINS \'{0}\' {1}{2}'.\
                                format(marker_rus, el['close_parenth'], cond)
                        else:
                            marker_lengh = str(len(marker_rus.split())+1)
                            if len(marker_rus.split()) > 1:
                                request_on_db += ' REDUCE(s = " ", w in split(n.text_norm, " ")' \
